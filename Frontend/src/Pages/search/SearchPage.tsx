...
const SearchPage = () => {
    ...
    const { searchResults, isLoading, error, fetchSearchResults } = useMusicStore();
                    ...
                    <TableBody>
                        {searchResults.length > 0 ? (
                            searchResults.map((result) => (
                                ...
                                >
                                    <TableCell>
                                        <img src={result.imageUrl} alt={result.title} className="size-10 rounded object-cover" />
                                    </TableCell>
                                    <TableCell className="font-medium">{result.title}</TableCell>
                                    <TableCell>{result.artist}</TableCell>
                                    <TableCell>
                                        <span className="inline-flex items-center gap-1 text-zinc-400">
                                            <Calendar className="h-4 w-4" /> {result.createdAt.split("T")[0]}
                                        </span>
                                    </TableCell>
                                    <TableCell>
